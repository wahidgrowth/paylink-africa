...
export default function PaymentPage() {
  ...
  const detectOperator = (phone: string) => {
    ...
    if (['70', '75', '76', '77', '78'].includes(prefix)) return 'Wave'
    return ''
  }
  ...
  if (step === 'success') return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ textAlign: 'center', padding: '40px' }}>
        <div style={{ width: '64px', height: '64px', background: '#10B98120', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: '28px', color: '#10B981' }}>✓</div>
      ...
      <div style={{ maxWidth: '420px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#111111', borderRadius: '16px', overflow: 'hidden', border: '0.5px solid #1F1F1F' }}>
          ...
          <div style={{ padding: '24px' }}>
            ...
            {step === 'page' && (
              <button onClick={() => setStep('checkout')} style={{ width: '100%', background: '#10B981', border: 'none', color: '#000', fontSize: '15px', fontWeight: '700', padding: '14px', borderRadius: '10px', cursor: 'pointer' }}>Payer maintenant</button>
            )}
            {step === 'checkout' && (
              <div>
                ...
                <button onClick={() => setStep('success')} style={{ width: '100%', background: '#10B981', border: 'none', color: '#000', fontSize: '15px', fontWeight: '700', padding: '14px', borderRadius: '10px', cursor: 'pointer' }}>Payer {total.toLocaleString()} FCFA</button>
